...
const UserInfoForm = (props: UserInfoFormProps) => {
  ...
  const { state: phone, bindings: phoneBindings } = useInput(
    initialValues.phone
  );
  const { state: country, bindings: countryBindings } = useInput(
    initialValues.country
  );
  const { selected: gender, bindings: genderBindings } = useRadioInput(
    ["M", "F"],
    initialValues.gender
  );

  const countries = useMemo(
    () => ["PAKISTAN", "India", "USA", "China", "UK"],
    []
  );
  ...
  return (
    <>
      <Plate>
        <h1>
          {action === "EDIT" ? "Edit Person's Information" : "Add new user"}
        </h1>
        <NavigationButton
          onClick={() => {
            navigate("/");
      ...
      <Form onSubmit={_handleFormSubmission}>
        ...
        <input
          ...
          required
          {...nameBindings}
          pattern="[A-Za-z]{3,15} [A-Za-z]{3,15}"
        ...
        <select required {...countryBindings}>
          ...
          {countries.map((_country, _index) => (
            <option value={_country} key={_index}>
              {_country}
            </option>
          ))}
        </select>
        <div className="buttonsWrapper">
          {action === "EDIT" ? (
            <button type="submit">Update Person's Information</button>
          ) : (
